...
<body>
    <p class="Parrafo">
        CONTRATO<strong>
            ...
            @else
                ABIERTO
            @endif
        </strong>PARA LA PRESTACIÓN DE SERVICIOS DE <strong>{{ $contrato->descripcion_contrato }}</strong>, CON CARÁCTER
        @if ($requisicion->pais_id == 1)
            <strong>NACIONAL</strong>
        @else
            <strong>INTERNACIONAL BAJO COBERTURA DE LOS TRATADOS</strong>
        @endif QUE CELEBRAN POR UNA PARTE, EL EJECUTIVO FEDERAL POR CONDUCTO DEL <strong>INSTITUTO
            DE SEGURIDAD Y SERVICIOS SOCIALES DE LOS TRABAJADORES DEL ESTADO</strong>, A QUIEN EN LO SUCESIVO SE LE
        DENOMINARÁ <strong>“LA ENTIDAD”</strong>, REPRESENTADO POR LA <strong>{{ $empleadosubdelegado->nombre }}
            {{ $empleadosubdelegado->apellido_paterno }} {{ $empleadosubdelegado->apellido_materno }}</strong>,
        <strong> {{ $empleadosubdelegado->Cargos->nombre_cargo }}</strong>, POR LA OTRA,<strong>
            {{ $persona->nombre_proveedor }}</strong>, EN LO SUCESIVO “EL PROVEEDOR”, @if ($persona !== null)
            @if ($persona->tipo_persona_id == 2)
                REPRESENTADA POR<strong> {{ $persona->representante_nombre }}</strong>, EN SU CARÁCTER DE
                <strong>{{ $persona->Caracter->nombre_tipo_caracter }}</strong>
            @endif
        @endif
        REPRESENTADO POR EL ADMINISTRADOR ÚNICO <strong>{{ $contrato->AdminContratos->nombre }}
            {{ $contrato->AdminContratos->apellido_paterno }}
            {{ $contrato->AdminContratos->apellido_materno }}</strong>, A QUIENES DE MANERA CONJUNTA SE LESDENOMINARÁ
        <strong>“LAS PARTES”</strong>, ALTENOR DE LAS DECLARACIONES Y CLÁUSULAS SIGUIENTES:
    </p>
    <h5 class="Titulo">
        <strong>DECLARACIONES</strong>
    </h5>
    <p class="SubTitulo">I. <strong>“LA DEPENDENCIA”</strong> DECLARA QUE:
    </p>
    <p class="Parrafo">I.1. QUE ES UN ORGANISMO DESCENTRALIZADO DE LA ADMINISTRACIÓN PÚBLICA FEDERAL, CON PERSONALIDAD
        JURÍDICA Y PATRIMONIO PROPIOS, DE CONFORMIDAD CON LO DISPUESTO POR LOS ARTÍCULOS 1°, 3°, FRACCIÓN I, Y 45 DE LA
        LEY ORGÁNICA DE LA ADMINISTRACIÓN PÚBLICA FEDERAL, 5° DE LA LEY FEDERAL DE ENTIDADES PARAESTATALES, ASÍ COMO 5º,
        207 Y 208 DE LA LEY DEL INSTITUTO DE SEGURIDAD Y SERVICIOS SOCIALES DE LOS TRABAJADORES DEL ESTADO. </p>

    <p class="Parrafo">
        I.2. SU REPRESENTANTE, LA <strong>{{ $empleadosubdelegado->nombre }}
            {{ $empleadosubdelegado->apellido_paterno }} {{ $empleadosubdelegado->apellido_materno }}</strong>, SE
        ENCUENTRA FACULTADA
        PARA SUSCRIBIR EL PRESENTE INSTRUMENTO LEGAL EN REPRESENTACIÓN DEL INSTITUTO DE SEGURIDAD Y SERVICIOS SOCIALES
        DE LOS TRABAJADORES DEL ESTADO, DE CONFORMIDAD CON LO ESTABLECIDO EN LA ESCRITURA PÚBLICA NÚMERO
        46,460, LIBRO 1,265 DE FECHA DIECIOCHO DE JULIO DEL AÑO DOS MIL VEINTIDÓS, PASADA ANTE LA FE DEL
        LICENCIADO ALBERTO T. SÁNCHEZ COLÍN, NOTARIO PÚBLICO NÚMERO 83 DE LA CIUDAD DE MÉXICO, QUIEN PODRÁ
        SER SUSTITUIDO EN CUALQUIER MOMENTO EN SU CARGO O FUNCIONES, SIN QUE ELLO IMPLIQUE LA NECESIDAD DE
        ELABORAR CONVENIO MODIFICATORIO.
    </p>
    <p class="Parrafo">
        I.3. DE CONFORMIDAD CON EL ARTÍCULO 26, APARTADO B, FRACCIÓN I Y X, DEL REGLAMENTO
        ÓRGANICO DE LAS DELEGACIONES ESTATALES Y REGIONALES DEL INSTITUTO DE SEGURIDAD Y SERVICIOS SOCIALES
        DE LOS TRABAJADORES DEL ESTADO SUSCRIBE EL PRESENTE INSTRUMENTO LEGAL COMO ADMINISTRADOR
        DEL CONTRATO
        EL <strong>{{ $contrato->AdminContratos->nombre }}
            {{ $contrato->AdminContratos->apellido_paterno }}{{ $contrato->AdminContratos->apellido_materno }}</strong>,
        <strong>{{ $contrato->AdminContratos->Cargos->nombre_cargo }}</strong>
        ,REGISTRO FEDERAL DE CONTRIBUYENTES <strong>{{ $contrato->AdminContratos->rfc }}</strong>, FACULTADO PARA
    ...
    <p class="Parrafo">
        I.4. DE CONFORMIDAD CON LOS ARTÍCULOS 34 Y 37 FRACCIONES I Y XIII DEL REGLAMENTO ÓRGANICO
        DE LAS DELEGACIONES ESTATALES Y REGIONALES DEL INSTITUTO DE SEGURIDAD Y SERVICIOS SOCIALES
        DE LOS TRABAJADORES DEL ESTADO, SUSCRIBE EL PRESENTE INSTRUMENTO LEGAL,
        <strong>{{ $empleadomateriales->nombre }} {{ $empleadomateriales->apellido_paterno }}
            {{ $empleadomateriales->apellido_materno }}</strong>,
        <strong>{{ $empleadomateriales->Cargos->nombre_cargo }}</strong>
        , REGISTRO FEDERAL DE CONTRIBUYENTES <strong>{{ $empleadomateriales->rfc }}</strong>
        FACULTADA PARA REALIZAR LA CONTRATACIÓN DE LOS SERVICIOS GENERALES Y SUPERVISIÓN DE LOS SERVICIOS REQUERIDOS
    ...
    <p class="Parrafo">
        ...
        <strong>{{ $requisicion->Tipos->nombre_tipo }}</strong>,
        ...
        ESTADOS UNIDOS MEXICANOS; <strong>ARTÍCULOS 26 FRACCIÓN III, 40 SEGUNDO Y TERCER PÁRRAFOS;
            42 PRIMER Y TERCER PÁRRAFO DE LA LEY DE ADQUISICIONES, ARRENDAMIENTOS Y SERVICIOS DEL
            SECTOR PÚBLICO.</strong>
    </p>
    <p class="Parrafo">
        I.6. <strong>“LA DEPENDENCIA”</strong> CUENTA CON SUFICIENCIA PRESUPUESTARIA OTORGADA MEDIANTE OFICIO
        <strong>{{ $contrato->oficio_suficiencia }}</strong>, DE
        ...
        <strong>{{ $empleadofinanzas->nombre }} {{ $empleadofinanzas->apellido_paterno }}
            {{ $empleadofinanzas->apellido_materno }}, {{ $empleadofinanzas->Cargos->nombre_cargo }}.</strong>
    </p>
    @if ($requisicion->pluralidad == 1)
        <p class="Parrafo">
            I.7 La SHCP (SECRETARIA DE HACIENDA Y CRÉDITO PÚBLICO) AUTORIZÓ LA PLURIANUALIDAD MEDIANTE EL OFICIO NÚMERO
            DE OFICIO <strong>{{ $contrato->oficio_plurianualidad }}</strong>.
        </p>
        @if ($requisicion->autorizacion_previa == 1)
            <p class="Parrafo">
                I.8 EN CASO DE QUE SE TRATE DE UNA CONTRATACIÓN CUYA VIGENCIA INICIE EN EL
                ...
                LEY FEDERAL DE PRESUPUESTO Y RESPONSABILIDAD HACENDARIA Y 146 DE SU REGLAMENTO.
            </p>
            <p class="Parrafo">
                I.9. PARA EFECTOS FISCALES LAS AUTORIDADES HACENDARIAS DE CONFORMIDAD CON LA CONSTANCIADE SITUACIÓN
                FISCAL GENERADA POR EL SAT (SISTEMA DE ADMINISTRACIÓN TRIBUTARIA) LE HAN ASIGNADO EL REGISTRO FEDERAL DE
                CONTRIBUYENTES: <strong>ISS6001015A3</strong> .
            </p>
            <p class="Parrafo">
                I.10. TIENE ESTABLECIDO SU DOMICILIO EN <strong>CALLE AMAPOLAS, NÚMERO 100 COLONIA REFORMA, C.P 68050
                    OAXACA DE JUÁREZ, OAXACA</strong>, MISMO QUE SEÑALA PARA LOS FINES Y EFECTOS LEGALES DEL PRESENTE
                CONTRATO.
            </p>
            @if ($requisicion->garantia1_id == 2 || ($requisicion->garantia_2_id == 2 || $requisicion->garantia_3_id == 2))
                <p class="Parrafo">
                    I.11. DE LA REVISIÓN AL HISTORIAL DE CUMPLIMIENTO EN MATERIA DE CONTRATACIONES EN EL REGISTRO ÚNICO
                    DE CONTRATISTAS, SE ADVIERTE QUE <strong>“EL PROVEEDOR”</strong>CUENTA CON UN GRADO DE CUMPLIMIENTO
                    <strong>, POR LO QUE <strong>“LA DEPENDENCIA”</strong> DETERMINA PROCEDENTE EFECTUAR LA REDUCCIÓN
                        DEL MONTO DE LA GARANTÍA POR UN PORCENTAJE DE {{ $contrato->reducción }}.</strong>
                </p>
            @else
            @endif
        @else
            <p class="Parrafo">
                I.8. PARA EFECTOS FISCALES LAS AUTORIDADES HACENDARIAS DE CONFORMIDAD CON LA CONSTANCIADE SITUACIÓN
                FISCAL GENERADA POR EL SAT (SISTEMA DE ADMINISTRACIÓN TRIBUTARIA) LE HAN ASIGNADO EL REGISTRO FEDERAL DE
                CONTRIBUYENTES: <strong>ISS6001015A3</strong> .
            </p>
            <p class="Parrafo">
                I.9. TIENE ESTABLECIDO SU DOMICILIO EN <strong>CALLE AMAPOLAS, NÚMERO 100 COLONIA REFORMA, C.P 68050
                    OAXACA DE JUÁREZ, OAXACA</strong>, MISMO QUE SEÑALA PARA LOS FINES Y EFECTOS LEGALES DEL PRESENTE
                CONTRATO.
            </p>
            @if ($requisicion->garantia1_id == 2 || ($requisicion->garantia_2_id == 2 || $requisicion->garantia_3_id == 2))
                <p class="Parrafo">
                    I.10. DE LA REVISIÓN AL HISTORIAL DE CUMPLIMIENTO EN MATERIA DE CONTRATACIONES EN EL REGISTRO ÚNICO
                    DE CONTRATISTAS, SE ADVIERTE QUE <strong>“EL PROVEEDOR”</strong>CUENTA CON UN GRADO DE CUMPLIMIENTO
                    <strong>, POR LO QUE <strong>“LA DEPENDENCIA”</strong> DETERMINA PROCEDENTE EFECTUAR LA REDUCCIÓN
                        DEL MONTO DE LA GARANTÍA POR UN PORCENTAJE DE {{ $contrato->reducción }}.</strong>
                </p>
    ...
    @else
        <p class="Parrafo">
            I.7. PARA EFECTOS FISCALES LAS AUTORIDADES HACENDARIAS DE CONFORMIDAD CON LA CONSTANCIADE SITUACIÓN FISCAL
            GENERADA POR EL SAT (SISTEMA DE ADMINISTRACIÓN TRIBUTARIA) LE HAN ASIGNADO EL REGISTRO FEDERAL DE
            CONTRIBUYENTES: <strong>ISS6001015A3</strong> .
        </p>
        <p class="Parrafo">
            I.8. TIENE ESTABLECIDO SU DOMICILIO EN <strong>CALLE AMAPOLAS, NÚMERO 100 COLONIA REFORMA, C.P 68050 OAXACA
                DE JUÁREZ, OAXACA</strong>, MISMO QUE SEÑALA PARA LOS FINES Y EFECTOS LEGALES DEL PRESENTE CONTRATO.
        </p>
        @if ($requisicion->garantia1_id == 2 || ($requisicion->garantia_2_id == 2 || $requisicion->garantia_3_id == 2))
            <p class="Parrafo">
                I.9. DE LA REVISIÓN AL HISTORIAL DE CUMPLIMIENTO EN MATERIA DE CONTRATACIONES EN EL REGISTRO ÚNICO DE
                CONTRATISTAS, SE ADVIERTE QUE <strong>“EL PROVEEDOR”</strong>CUENTA CON UN GRADO DE CUMPLIMIENTO
                <strong>, POR LO QUE <strong>“LA DEPENDENCIA”</strong> DETERMINA PROCEDENTE EFECTUAR LA REDUCCIÓN DEL
    ...
    @if ($personafisica = null)
        <p>
            II.1 ES UNA PERSONA MORAL LEGALMENTE CONSTITUIDA DE CONFORMIDAD CON LA LEGISLACIÓN MEXICANA, SEGÚN CONSTA EN
            EL TESTIMONIO QUE CONTIENE
            {{ $personamoral->instrumento_publico }}, CON EL FOLIO {{ $personamoral->fiolio_registro }}, DE FECHA
            {{ $personamoral->fecha_registro }}, PASADA ANTE LA FE DEL
            {{ $personamoral->registro_publico }}.
        </p>
    @else
        <p>
            II.1. ES UNA PERSONA FÍSICA, DE NACIONALIDAD <strong>{{ $contrato->nacionalidad }}</strong> LO QUE ACREDITA
            CON {{ $contrato->documento_expedicion }}, EXPEDIDA POR {{ $contrato->instutucion_expedida }}.
        </p>
    @endif
    <p>
        II.2. REÚNE LAS CONDICIONES TÉCNICAS, JURÍDICAS Y ECONÓMICAS, Y CUENTA CON LA ORGANIZACIÓN Y ELEMENTOS
    ...
    <p>
        II.4. BAJO PROTESTA DE DECIR VERDAD, ESTÁ AL CORRIENTE EN LOS PAGOS DE SUS OBLIGACIONES FISCALES, EN ESPECÍFICO
        LAS PREVISTAS EN EL ARTÍCULO 32-D DEL CÓDIGO FISCAL FEDERAL VIGENTE, ASÍ COMO DE SUS OBLIGACIONES FISCALES EN
        MATERIA DE SEGURIDAD SOCIAL, ANTE INSTITUTO DEL FONDO NACIONAL DE LA
        VIVIENDA PARA LOS TRABAJADORES (INFONAVIT) Y INSTITUTO MEXICANO DEL SEGURO SOCIAL (IMSS); LO QUE ACREDITA CON
        LAS OPINIONES DE CUMPLIMIENTO DE OBLIGACIONES FISCALES Y EN MATERIA DE SEGURIDAD
        SOCIAL EN SENTIDO POSITIVO, EMITIDAS POR EL SAT E IMSS, RESPECTIVAMENTE, ASÍ COMO CON LA CONSTANCIA DE SITUACIÓN
        FISCAL EN MATERIA DE APORTACIONES PATRONALES Y ENTERO DE DESCUENTOS, SIN
        ADEUDO, EMITIDA POR EL INFONAVIT, LAS CUALES SE ENCUENTRAN VIGENTES Y OBRAN EN EL EXPEDIENTE RESPECTIVO.
    </p>
    <p>
        II.5. TIENE ESTABLECIDO SU DOMICILIO EN <strong>{{ $persona->domicilio }}</strong> MISMO QUE SEÑALA PARA LOS
        FINES Y EFECTOS LEGALES DEL PRESENTE CONTRATO.
    </p>

    <p class="SubTitulo"><strong>De “LAS PARTES”:</strong></p>
    <p>
        II.6. QUE ES SU VOLUNTAD CELEBRAR EL PRESENTE CONTRATO Y SUJETARSE A SUS TÉRMINOS Y
        CONDICIONES, PARALO CUAL SE RECONOCEN LAS FACULTADES Y CAPACIDADES, MISMAS QUE NO LES HAN SIDO REVOCADAS O
        LIMITADAS EN FORMA ALGUNA, POR LO QUE DE COMÚN ACUERDO SE OBLIGAN DE CONFORMIDAD CON LAS SIGUIENTES:
    </p>
    ...
    <p class="Parrafo">
        @if ($requisicion->tipo_id == 1 && $requisicion->pluralidad == 0)
            ...
            CONTRATO, LA CANTIDAD DE <strong>${{ $requisicion->total }}, IVA INCLUIDO</strong>, MENOS LAS RETENCIONES
            QUE CONFORME A LA LEY PROCEDAN.
        @else
            “LA DEPENDENCIA” PAGARÁ A “EL PROVEEDOR” COMO CONTRAPRESTACIÓN POR LOS SERVICIOS OBJETO DE ESTE
            CONTRATO, LA CANTIDAD DE <strong>${{ $requisicion->total }}, IVA INCLUIDO</strong>, IMPORTE QUE SE CUBRIRÁ
            EN CADA UNO DE LOS EJERCICIOS FISCALES, DE ACUERDO A LO SIGUIENTE:
            <div>
                <?php
    ...
    <div>
        ...
        $table = $section->addTable();
        $table->addRow();
        $table->addCell(500)->addText('No. de partida');
        $table->addCell(500)->addText('CUCOP');
        $table->addCell(500)->addText('Descripción');
        $table->addCell(500)->addText('Cantidad solicitada');
        $table->addCell(500)->addText('Unidad de medida');
        $table->addCell(500)->addText('Precio unitario');
        $table->addCell(500)->addText('Importe');
        
        foreach ($requisicion->detalles as $detalle) {
            $table->addRow();
            $table->addCell()->addText($detalle->num_partida);
            $table->addCell()->addText($detalle->cucop);
            $table->addCell()->addText($detalle->Insumos->descripcion);
            $table->addCell()->addText($detalle->cantidad);
            $table->addCell()->addText($detalle->Medidas->nombre_medida);
            $table->addCell()->addText('$' . $detalle->precio);
            $table->addCell()->addText('$' . $detalle->importe);
        }
    ...
    @else
        ...
        // Aquí puedes iterar sobre los datos y agregar filas con los valores correspondientes
        // Ejemplo:
        foreach ($requisicion->detalles as $detalle) {
            $table->addRow();
            $table->addCell()->addText($detalle->num_partida);
    ...
    <ol>
        <li>
            CONTRAVENIR LOS TÉRMINOS PACTADOS PARA LA PRESTACIÓN DE LOS SERVICIOS, ESTABLECIDOS EN EL
            PRESENTE CONTRATO.
        </li>
        <li>
        ...
        <li>
            SUSPENDER TOTAL O PARCIALMENTE Y SIN CAUSA JUSTIFICADA LA PRESTACIÓN DE LOS SERVICIOS DEL
            PRESENTE CONTRATO.
        </li>
        <li>
            NO REALIZAR LA PRESTACIÓN DE LOS SERVICIOS EN TIEMPO Y FORMA CONFORME A LO ESTABLECIDO EN EL
            PRESENTE CONTRATO Y SUS RESPECTIVOS ANEXOS.
        </li>
        <li>
    ...
    <div>
        <table class="tabla">
            ...
            <tbody>
                <tr>
                    <td class="cuerpotabla">{{ $empleadosubdelegado->nombre }} {{ $empleadosubdelegado->apellido_paterno }}
                        {{ $empleadosubdelegado->apellido_materno }}
                    </td>
                ...
                <tr>
                    <td>{{ $empleadomateriales->nombre }} {{ $empleadomateriales->apellido_paterno }}
                        {{ $empleadomateriales->apellido_materno }}
                    </td>
